...
const ThemeSelectionScreen = () => {
    ...
    const currentThemeId = useSelector((state: RootState) => state.theme.currentTheme);
    ...
    const renderThemeItem = ({ item }: { item: typeof themes[0] }) => {
        ...
        return (
            <TouchableOpacity onPress={() => handleSelectTheme(item.id)} activeOpacity={0.8}>
                <Card
                    style={[
                        styles.themeCard,
                        {
                            borderColor: isSelected ? theme.colors.primary : item.colors.border,
                ...
                >
                    <View style={styles.cardContent}>
                        <View style={[styles.previewCircle, { backgroundColor: item.colors.primary }]} />
                        <View style={{ flex: 1 }}>
                            ...
                            <View style={{ flexDirection: 'row', gap: 4, marginTop: 4 }}>
                                ...
                                <View style={[styles.colorDot, { backgroundColor: item.colors.surface }]} />
                            </View>
                        </View>
                        {isSelected && (
    ...
    if (user?.role === 'client') {
        return (
            <View style={[styles.container, { justifyContent: 'center', alignItems: 'center', padding: 20 }]}>
                <Icon name="lock-closed-outline" size={48} color={theme.colors.textSecondary} />
                <Text style={{ marginTop: 16, fontSize: 18, color: theme.colors.text, textAlign: 'center' }}>
                    Access Restricted
                </Text>
                ...
                <Button title="Go Back" onPress={() => navigation.goBack()} style={{ marginTop: 24, width: '100%' }} />
            </View>
        );
    }
